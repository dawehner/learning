...
var $author$project$Main$Ip4 = function (a) {
	return {$: 'Ip4', a: a};
};
var $elm$parser$Parser$Loop = function (a) {
	return {$: 'Loop', a: a};
};
var $author$project$Main$Ipv4 = F4(
...
var $author$project$Main$checkRange = F3(
	function (from, to, string) {
		var _v0 = $elm$core$String$toInt(string);
		if (_v0.$ === 'Nothing') {
			return $elm$parser$Parser$problem('String is not an integer');
		} else {
			var x = _v0.a;
			return (_Utils_cmp(x, to) > 0) ? $elm$parser$Parser$problem('Number too big') : ((_Utils_cmp(x, from) < 0) ? $elm$parser$Parser$problem('Number too small') : $elm$parser$Parser$succeed(x));
		}
	});
...
var $author$project$Main$intRange = F2(
	function (from, to) {
		return A2(
			$elm$parser$Parser$andThen,
			A2($author$project$Main$checkRange, from, to),
...
var $author$project$Main$ip4Parser = A2(
	...
	A2(
		...
		A2(
			...
			A2(
				$elm$parser$Parser$keeper,
				$elm$parser$Parser$succeed($author$project$Main$Ipv4),
				A2(
					$elm$parser$Parser$ignorer,
					A2($author$project$Main$intRange, 0, 255),
...
var $author$project$Main$keepLineComment = function (str) {
	return A2(
		$elm$parser$Parser$keeper,
		A2(
			$elm$parser$Parser$ignorer,
			A2(
				$elm$parser$Parser$ignorer,
				$elm$parser$Parser$succeed($author$project$Main$Comment),
...
var $author$project$Main$hostFileParserHelper = function (hs) {
				...
				$elm$parser$Parser$succeed(
					function (entry) {
						return $elm$parser$Parser$Loop(
							A2($elm$core$List$cons, entry, hs));
					}),
				$elm$parser$Parser$oneOf(
					_List_fromArray(
						[
							...
							A2($elm$parser$Parser$map, $author$project$Main$Ip4, $author$project$Main$ip4Parser)),
							$author$project$Main$keepLineComment('#')
						]))),
				A2(
				$elm$parser$Parser$map,
				function (_v0) {
					return $elm$parser$Parser$Done(hs);
				},
...
var $author$project$Main$hostFileParser = A2(
	$elm$parser$Parser$ignorer,
	A2($elm$parser$Parser$loop, _List_Nil, $author$project$Main$hostFileParserHelper),
	$elm$parser$Parser$end);
...
var $author$project$Main$executeParseHostfile = function (string) {
	return A2($elm$parser$Parser$run, $author$project$Main$hostFileParser, string);
};
...
var $author$project$Main$hex = A2(
	...
	function (x) {
		switch (x) {
			...
			case 'C':
				return $elm$parser$Parser$succeed(12);
			case 'D':
				return $elm$parser$Parser$succeed(13);
			case 'E':
				return $elm$parser$Parser$succeed(14);
			case 'F':
				return $elm$parser$Parser$succeed(15);
			default:
				var y = x;
				var a = A2($elm$core$Debug$log, 'y', y);
				return $elm$parser$Parser$problem('invalid char');
		}
	},
	$elm$parser$Parser$getChompedString(
		A2(
...
var $author$project$Main$ip6Parser = A2(
	$elm$parser$Parser$keeper,
	A2(
		$elm$parser$Parser$keeper,
		A2(
					...
					function (_v0) {
						return A2(
							...
							$elm$core$Result$Ok(
								_List_fromArray(
									[
										$author$project$Main$Comment('Test')
									])),
							$author$project$Main$executeParseHostfile('# Test'));
					})
				])),
			A2(
			$elm_explorations$test$Test$describe,
				...
				[
					...
					function (_v1) {
						return A2(
							$elm_explorations$test$Expect$equal,
							$elm$core$Result$Ok(
								{k1: 127, k2: 0, k3: 0, k4: 1}),
							A2($elm$parser$Parser$run, $author$project$Main$ip4Parser, '127.0.0.1'));
					})
				])),
			A2(
			$elm_explorations$test$Test$describe,
				...
				[
					...
					function (_v2) {
						return A2(
							...
							A2($elm$parser$Parser$run, $author$project$Main$ip6Parser, '0000:0000'));
					})
				]))
		]));
var $author$project$Test$Generated$Main3441658185$main = A2(
	$author$project$Test$Runner$Node$run,
	{
		paths: _List_fromArray(
			['/Users/dawehner/Documents/Projects/learning/elm/block-host/tests/Example.elm']),
		processes: 8,
		report: $author$project$Test$Reporter$Reporter$ConsoleReport($author$project$Console$Text$UseColor),
		runs: $elm$core$Maybe$Nothing,
		seed: 331629002259628
	},
...
_Platform_export({'Test':{'Generated':{'Main3441658185':{'init':$author$project$Test$Generated$Main3441658185$main($elm$json$Json$Decode$int)(0)}}}});}(this));
return this.Elm;
})({});
var pipeFilename = "/tmp/elm_test-60077.sock";
