...
(|||) (P p1) (P p2) =
  P (\input ->
      let
        ...
        p2Result = p2 input
      in
        if isErrorResult p1Result then p2Result else p1Result
    )
...
instance Monad Parser where
  ...
  (=<<) f (P p) =
    P (\input ->
       onResult (p input) (\input' x -> parse (f x) input')
      )
...
satisfy pred =
  P (\input -> case input of
    Nil -> UnexpectedEof
    (x :. xs) -> if pred x then Result xs x else UnexpectedChar x
  )
...
sequenceParser ::
  List (Parser a)
  -> Parser (List a)
sequenceParser = foldRight (\px pxs -> (\x -> (\xs -> pure $ x :. xs) =<< pxs) =<< px) (pure Nil)
...
firstNameParser ::
  Parser Chars
firstNameParser = upper >>= \x -> lift2 (:.) (pure x) (list lower)
  ...
  ageParser >>= \age ->
    firstNameParser >>=~ \firstName ->
        surnameParser >>=~ \surname ->
            smokerParser >>=~ \smoker ->
                phoneParser >>=~ \phone -> pure $ Person age firstName surname smoker phone
