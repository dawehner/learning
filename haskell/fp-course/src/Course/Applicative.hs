...
instance Applicative ExactlyOne where
  ...
  (<*>) ::
    ...
    -> ExactlyOne a
    -> ExactlyOne b
  (<*>) ff fx = bindExactlyOne (\f -> bindExactlyOne (\x -> pure (f x)) fx) ff
...
instance Applicative Optional where
  ...
  (<*>) ::
    ...
    -> Optional a
    -> Optional b
  (<*>) ff fx = bindOptional (\f -> bindOptional (\x -> pure (f x)) fx) ff
...
replicateA ::
  ...
  -> f a
  -> f (List a)
replicateA n fx = sequence $ take n $ repeat fx
...
filtering ::
  ...
  -> List a
  -> f (List a)
filtering fp xs = foldRight (\x fx -> lift2 (\b xs_ -> if b then x :. xs_ else xs_) (fp x) fx) (pure Nil) xs
