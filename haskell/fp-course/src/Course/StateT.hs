...
instance Functor f => Functor (StateT s f) where
  (<$>) ::
    ...
    -> StateT s f a
    -> StateT s f b
  (<$>) f StateT { runStateT = rst } = StateT (\s -> 
      (\(a, s') -> (f a, s')) <$> rst s
    )
...
instance Monad f => Applicative (StateT s f) where
  ...
  (<*>) ::
    ...
    -> StateT s f a
    -> StateT s f b
  (<*>) StateT { runStateT = fsf } StateT { runStateT = fsa } =
    StateT (\s -> (\(f, s') -> (\(a, s'') -> pure $ (f a, s'')) =<< fsa s') =<< fsf s)
...
instance Monad f => Monad (StateT s f) where
  (=<<) ::
    ...
    -> StateT s f a
    -> StateT s f b
  (=<<) f StateT { runStateT = fsa } =
    StateT (\s -> (\(x, s') -> runStateT (f x) s') =<< fsa s)
...
runState' ::
  ...
  -> s
  -> (a, s)
runState' StateT { runStateT = sa } s = runExactlyOne $ sa s
...
execT ::
  ...
  -> s
  -> f s
execT StateT { runStateT = sfa } s = snd <$> sfa s
...
evalT ::
  ...
  -> s
  -> f a
evalT StateT { runStateT = sfa } s = fst <$> sfa s
...
distinct' ::
  ...
  -> List a
distinct' xs =
 eval' (filtering (\x -> state' (\s -> (not $ S.member x s, S.insert x s))) xs) S.empty
  ...
  runOptionalT $ evalT (filtering (\x -> StateT (\s -> 
    OptionalT $
      if x > 100
        ...
        then log1 (fromString ("even number: " P.++ show x)) $ Full (not $ S.member x s, S.insert x s)
      else
        pure $ Full (not $ S.member x s, S.insert x s)
   )) xs) S.empty
