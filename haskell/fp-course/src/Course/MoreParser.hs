...
string ::
  Chars
  -> Parser Chars
string chars = (traverse charTok $ reverse chars) >>= (pure . reverse)
...
between ::
  ...
  -> Parser a
  -> Parser a
between beforeParser afterParser valueP =
  beforeParser >>= \_ ->
    valueP >>= \value -> 
      afterParser >>= \_ -> pure value
...
betweenCharTok ::
  ...
  -> Parser a
  -> Parser a
betweenCharTok beforeChar afterChar parser = between (charTok beforeChar) (charTok afterChar) parser
...
hex =
  let
    hInt s = case readHex s of
      Empty -> 0
      Full n -> n
  in
    chr . hInt <$> reverse <$> replicateA 4 (satisfy isHexDigit)
...
satisfyAll ::
  List (Char -> Bool)
  -> Parser Char
satisfyAll conds = (\preds -> satisfy (and . preds)) $ sequence conds
...
satisfyAny ::
  List (Char -> Bool)
  -> Parser Char
satisfyAny conds = (\preds -> satisfy (or . preds)) $ sequence conds
...
betweenSepbyComma ::
  ...
  -> Parser a
  -> Parser (List a)
betweenSepbyComma start end parser =
  betweenCharTok start end (sepby parser (charTok ','))
